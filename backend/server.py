...
- يمكنك الإجابة عن: الطلبات، الدفع، التوصيل، المتاجر، المنتجات، الحساب، الإحالات"""
    ...
    if GEMINI_API_KEY:
        ...
        # نماذج فلاش المجانية قد تكون مشغولة لحظياً (503/429) فنحاولها بالترتيب.
        gemini_contents = []
        for turn in recent_chat:
        ...
        candidate_models = [GEMINI_MODEL] + GEMINI_FALLBACK_MODELS
        last_gm_error = ""
        for mdl in candidate_models:
            try:
                ...
                if gm_resp.status_code == 200 and gm_json.get("candidates"):
                    ...
                    if response_text:
                        break
                last_gm_error = (f"{mdl}: HTTP {gm_resp.status_code} "
                                 f"{gm_json.get('error', {}).get('message', '')}")
            except Exception as e:
                last_gm_error = f"{mdl}: {e}"
        if not response_text:
            logger.warning(f"Gemini API error: {last_gm_error}")
